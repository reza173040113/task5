...
class _HalamanProfileState extends State<HalamanProfile> {
  ...
  Future<List> getData() async {
    ...
    String id_user = prefs.getString("id_user");//mungkn  kemarin kita ngantuk//wwkwk mungkin kakhaha
...
//apa gara2 logout tadi blm login lagi atau gimana ya?emg tadi abis logout lngsung bbisa masuk lagi y? nah tadi lupa kak setau saya tadi abis logout trs direstart//apa saya hapus data dulu aja?coba aja
    final response =
    await http.post("http://192.168.18.4/flutter-server/getUserById.php", body: {
      "id_user": id_user ,//ini kak kalo diganti pake iduser error l,dicoba run a?
                  ...
                  return new Container(
                    child: new GestureDetector(
                      ...
                      child: new Card(
                        color: Colors.white,
                        child: new ListTile(
                          title: new Text(widget.list[i]['username']),
                          leading: new Icon(Icons.list),
                          subtitle: new Text("Jenis :${widget.list[i]['username']}"),
                        ),
                      ),
                    ),
                  );
